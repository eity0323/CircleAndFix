...
public class DatasUtil {
	public static final String[] CONTENTS = { "", "哈哈", "今天是个好日子", "呵呵", "图不错",
			"我勒个去","我就是测试测试","请叫我冒泡君" };
    public static final String[] VIDEOS = {"video_sample_1.mp4","video_sample_2.mp4","video_sample_3.mp4","video_sample_4.mp4"};
	...
	public static List<CircleItem> createCircleDatas() {
		...
		for (int i = 0; i < 15; i++) {
			...
			} else if (getRandomNum(10) % 3 == 1) {
                item.setType("4");//视频
                item.setLinkImg(DatasUtil.VIDEOS[getRandomNum(3)]);
            } else{
				item.setType("2");// 图片
