...
*
 */
public class CircleListViewPublicCommentContral implements BasePublicCommentControl{
	private static final String TAG = CircleListViewPublicCommentContral.class.getSimpleName();
	...
	public CircleListViewPublicCommentContral(Context context, View editTextBody, EditText editText, View sendBt){
		mContext = context;
	...
	* @throws
	...
	public void handleListViewScroll() {
        ...
        if(mEditTextBody != null){
            editTextBodyH = mEditTextBody.getHeight();
        }else {
            editTextBodyH = DensityUtil.dip2px(mContext, 30);//mHelper.getEditTextBodyHeight();//整个EditTextBody的高度
        }
		int screenlH = (int)DensityUtil.getScreenHeight(mContext);//整个应用屏幕的高度
